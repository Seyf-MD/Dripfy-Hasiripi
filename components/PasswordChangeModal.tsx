    ...
    const renderContent = () => {
        if (step === 'success') {
            return (
                <div className="text-center p-8 flex flex-col items-center justify-center">
                    ...
                    <h3 className="text-xl font-bold text-black dark:text-white">{t('passwordChange.success')}</h3>
                </div>
            )
        }
        if (step === 'verify') {
            return (
                <form onSubmit={handleUpdatePassword} className="space-y-4">
                    <p className="text-sm text-center text-neutral-500 dark:text-neutral-400 -mt-2 mb-4">{t('passwordChange.subtitleCode').replace('{email}', email)}</p>
                    ...
                    <button type="submit" className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-[#32ff84] text-black text-sm font-semibold rounded-lg hover:bg-green-400 transition-colors">
        ...
        return ( // step === 'email'
            <form onSubmit={handleSendCode} className="space-y-4">
                 <p className="text-sm text-center text-neutral-500 dark:text-neutral-400 -mt-2 mb-4">{t('passwordChange.subtitleEmail')}</p>
                 ...
                 <button type="submit" className="w-full flex items-center justify-center px-4 py-2 bg-neutral-200 dark:bg-neutral-700 text-black dark:text-white text-sm font-semibold rounded-lg hover:bg-neutral-300 dark:hover:bg-neutral-600 transition-colors">
        ...
        >
            <div className="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 w-full max-w-sm flex flex-col" onClick={(e) => e.stopPropagation()}>
                <header className="p-4 flex justify-between items-center border-b border-neutral-200 dark:border-neutral-700">
                    <h2 id={modalTitleId} className="text-xl font-bold text-black dark:text-white">{t('passwordChange.title')}</h2>
