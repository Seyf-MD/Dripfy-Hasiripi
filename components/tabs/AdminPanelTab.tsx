                        ...
                        {users.map(user => (
                            <div key={user.id} onClick={() => onOpenModal(user, 'users')} className="bg-white dark:bg-neutral-800 p-4 rounded-lg border border-slate-200 dark:border-neutral-700 cursor-pointer hover:bg-slate-100 dark:hover:bg-neutral-700/50 transition-colors">
                                <div className="flex justify-between items-center">
                                    <div>
                                        <h3 className="font-semibold text-[var(--drip-text)] dark:text-white">{user.name}</h3>
                                        <p className="text-sm text-[var(--drip-muted)] dark:text-neutral-400">{user.email}</p>
                                    </div>
                                    <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${user.role === 'admin' ? 'bg-[color:rgba(75,165,134,0.15)] text-[var(--drip-primary)]' : 'bg-slate-200 dark:bg-neutral-700 text-[color:var(--drip-text-soft)] dark:text-neutral-300'}`}>
                            ...
                            <div key={req.id} className="bg-white dark:bg-neutral-800 p-4 rounded-lg border border-slate-200 dark:border-neutral-700">
                                <div className="flex flex-wrap justify-between items-start gap-4">
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        ...
                                        <button onClick={() => onApproveSignup(req.id)} className="p-2 rounded-full bg-[color:rgba(75,165,134,0.15)] hover:bg-[color:rgba(75,165,134,0.25)] text-[var(--drip-primary)] dark:text-[var(--drip-primary)] transition-colors"><CheckCircle size={20}/></button>
                                    </div>
                                </div>
                            </div>
                        )) : <p className="text-center text-[var(--drip-muted)] dark:text-neutral-400 py-8">{t('admin.noRequests')}</p>}
                    </div>
                );
            default:
                return null;
                ...
                <nav className="flex flex-row lg:flex-col gap-2">
                    {subTabs.map(tab => (
                        <button 
                            ...
                            className={`w-full flex items-center justify-between p-3 rounded-lg text-sm font-semibold transition-colors ${activeSubTab === tab.id ? 'bg-[var(--drip-primary)] text-white' : 'bg-slate-100 dark:bg-neutral-800 hover:bg-slate-200 dark:hover:bg-neutral-700'}`}
                        >
                            ...
                            {tab.count !== undefined && tab.count > 0 && (
                                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${activeSubTab === tab.id ? 'bg-white/30 text-white' : 'bg-[var(--drip-primary)] text-white'}`}>{tab.count}</span>
                            )}
                        </button>
                    ))}
                </nav>
